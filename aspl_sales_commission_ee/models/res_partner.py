...
class ResPartner(models.Model):
    ...
    is_sales_agent = fields.Boolean(string="Sales Agent")

    @api.constrains('comm_ids', 'comm_ids.commission')
    def _check_commission_values(self):
        if self.comm_ids.filtered(
                lambda line: line.compute_price_type == 'per' and line.commission > 100 or line.commission < 0.0):
