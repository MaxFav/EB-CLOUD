...
class report_account_aged_receivable_currency(models.AbstractModel):
    _name = "account.aged.receivable.currency"
    _description = "Aged Receivable By Currency"
    _inherit = "account.aged.partner"

    def _set_context(self, options):
        ctx = super(report_account_aged_receivable_currency, self)._set_context(options)
    ...
    def _get_templates(self):
        templates = super(report_account_aged_receivable_currency, self)._get_templates()
    ...
    def _get_lines(self, options, line_id=None):
        ''' Inherit to format amount in partner's currency. '''
        sign = -1.0 if self.env.context.get('aged_balance') else 1.0
        lines = []
        account_types = [self.env.context.get('account_type')]
        results, total, amls = self._get_partner_move_lines(account_types, self._context['date_to'], 'posted', 30)

        for values in results:
            if line_id and 'partner_%s' % (values['partner_id'],) != line_id:
                continue

            currency_id = self.env['res.partner'].browse([values['partner_id']])[0] \
            ...
            vals = {
                ...
                'name': values['name'],
                'level': 2,
                'columns': [{'name': ''}] * 3 + [{'name': self.format_value(sign * v, currency_id), 'currency_id': currency_id} for v in
                ...
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in
                                   ...
                                   [{'name': v} for v in
                                    [line['period'] == 6 - i and self.format_value(sign * line['amount'],
                                                                                   currency_id) or '' for i in
                                     range(7)]],
    ...
    def _get_cell_type_value(self, cell):
        ...
        return super(report_account_aged_receivable_currency, self)._get_cell_type_value(cell)

    def _ignore_line_amount(self, line_amount):
        return 0.05 > line_amount > -0.05
