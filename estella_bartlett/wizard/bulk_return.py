...
class BulkReturn(models.TransientModel):
    ...
    def button_return(self):
        if not self.return_line_ids:
            raise UserError(_("Products must be entered into the wizard to be returned before a bulk return can be processed."))
        ...
        for line in self.return_line_ids:
            ...
            # quantity, state, picking type, not being linked to an existing return, and being from an order within the last 18 months.
            matching_lines = self.env['stock.move'].search([('product_id', '=', line.returned_product_id.id),
                                                            ('sale_line_id.order_id.partner_invoice_id', '=', self.partner_id.id),
                                                            ('state', '=', 'done'),
                                                            ('picking_type_id.code', '=', 'outgoing'),
                                                            ('sale_line_id.order_id.date_order', '>=', datetime.today() - relativedelta(months=18))], order="id desc")
        ...
        error_msg = ""
        if error_products:
            error_msg += "The following product(s) cannot be found at all on previous sales within the past 18 months " \
                         "for this customer: "
            error_prodstr = ', '.join(error_products)
            error_msg += error_prodstr + "\n"
        if insufficient_qty_products:
            error_msg += "The following product(s) cannot be found with the provided quantities in previous sales " \
        ...
        for picking in matching_pickings_moves:
            return_picking_wizard = self.with_context(active_id=picking.get('picking'), bulk_return=picking.get('moves')).env['stock.return.picking'].sudo().create({
            ...
            immediate_transfer_dict = returned_picking.button_validate_limit()
            if immediate_transfer_dict.get('res_model') == 'stock.immediate.transfer' and immediate_transfer_dict.get(
                    'res_id'):
                immediate_transfer_wizard = self.env['stock.immediate.transfer'].browse(
                    immediate_transfer_dict.get('res_id'))
            ...
            for move in (scrap_move for scrap_move in picking.get('moves') if scrap_move.get('scrap') is True):
                ...
                if scrap_product_dict:
                    ...
                    scrap_product_record = self.with_context(scrap_product_dict_context).env['stock.scrap'].create({
        ...
        matching_sales = self.env['sale.order'].browse(set([picking['sale'] for picking in matching_pickings_moves]))
        credit_note = matching_sales.create_bulk_return_credit_note(final=True, lines=matching_sale_orders_lines)
...
class BulkReturnLine(models.TransientModel):
    ...
    def validate_quantity(self):
        for line in self:
            if line.quantity <= 0:
                raise UserError(_("The quantity of product returned on each line must be above 0."))
